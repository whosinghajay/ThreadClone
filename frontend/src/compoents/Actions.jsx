...
const Actions = ({ post }) => {
  ...
  const handleLikeAndUnlike = async () => {
    ...
    try {
      ...
      if(!liked){
        //add the id of the cuurent user to post.like array
        ...
        //remove the id of the current user from post.like array
        const updatedPosts = posts.map((p) => {
					if (p._id === post._id) {
  ...
  const handleReply = async() => {
    if(!user) return showToast("Error" , "You must be logged in to reply on a post" , "error");
    ...
    <Flex gap={3} my={2} onClick={(e) => e.preventDefault()}>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme='blue' size={"sm"} mr={3} isLoading={isReplying } onClick={handleReply}>Reply</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
